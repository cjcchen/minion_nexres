...
if ($RUNNER->run($FLEET, [ 'deploy-poa-poc' ])->wait() != 0) {
    die ("failed to deploy poa-poc");
}
...
if ($RUNNER->run($FLEET, [ 'deploy-quorum-ibft-poc' ])->wait() != 0) {
    die ("failed to deploy quorum-ibft-poc");
}
